...
EmpresaUm::Empresa::Empresa():financas("financas","administrador", "contador", "economista")   // Sempre que se constroi uma empresa ela tera esses cinco setores que serao inicializados com seus titulos e as profissoes que
...
,design("design","design grafico", "design multimedia", "eng. social")
{
    ...
    Profissoes.push_back("design gr�fico");
...
EmpresaUm::Empresa::~Empresa()
{
    //dtor
}
        void EmpresaUm::Empresa::SetTotaldeFuncionarios()   //Conta quantos empregados cadastrados a empresa tem. Acessa todas as databases que guardam os empregados e conta quantos estao cadastrados
        {
            ...
            for(unsigned int i = 0; i<financas.Auxiliares.size();i++)        //Varre os vetores e vai somando cada elemento ou seja funcionario que encontra ao total
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<financas.Tecnicos.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<financas.Profissionais.size();i++)     //Para cada Setor ele varre as databases de Auxiliares, Tecnicos, Profissionais e Diretor
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<financas.Diretor.size();i++)
            {
                TotaldeFuncionarios++;
            }
            ...
            for(unsigned int i = 0; i<marketing.Auxiliares.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<marketing.Tecnicos.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<marketing.Profissionais.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<marketing.Diretor.size();i++)
            {
                TotaldeFuncionarios++;
            }
            ...
            for(unsigned int i = 0; i<tecnologia.Auxiliares.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<tecnologia.Tecnicos.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<tecnologia.Profissionais.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<tecnologia.Diretor.size();i++)
            {
                TotaldeFuncionarios++;
            }

            //Conta todos os funcionarios do setor NORMATIVIDADE
            for(unsigned int i = 0; i<normatividade.Auxiliares.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<normatividade.Tecnicos.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<normatividade.Profissionais.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<normatividade.Diretor.size();i++)
            {
                TotaldeFuncionarios++;
            }

            //Conta todos os funcionarios do setor DESIGN
            for(unsigned int i = 0; i<design.Auxiliares.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<design.Tecnicos.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<design.Profissionais.size();i++)
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<design.Diretor.size();i++)
            {
                TotaldeFuncionarios++;
            }

            //Varre tambem as databases que estao fora dos setores e soma os empregados
            for(unsigned int i = 0; i<DiretoresdeOperacao.size();i++)    //Diretores de Operacao
            {
                TotaldeFuncionarios++;
            }
            for(unsigned int i = 0; i<DiretorExecutivo.size();i++)       // Diretor Executivo
            {
                TotaldeFuncionarios++;
            }
                //No fim se tem a soma de todos os elementos
        }
        void EmpresaUm::Empresa::DemitirEmpregado(string id) //Procura por todos os setores, encontra o ID passado como argumento e deleta o funcionario da database, Caso nao encontre o ID reporta que nao existe funcionario com o id digitado
        {
          ...
          //Procura todos os funcionarios do setor FINANCAS e se encontra o id deleta o funcionario da database
            for(unsigned int i = 0; i<financas.Auxiliares.size();i++)        //Para cada setor e tipo de empregado os blocos funcionam da seguinte maneira
            ...
            for(unsigned int i = 0; i<financas.Tecnicos.size();i++)          //A mesma logica do primeiro for serve para todos os outros
            {
            ...
            for(unsigned int i = 0; i<financas.Profissionais.size();i++)
            {
           ...
           //Procura todos os funcionarios do setor MARKETING e se encontra o id deleta o funcionario da database-----------------------------------------------------
            ...
            for(unsigned int i = 0; i<DiretoresdeOperacao.size();i++)        //Fora dos Setores tambem eh checado pois eh possivel deletar Diretores
            {
            ...
            for(unsigned int i = 0; i<DiretorExecutivo.size();i++)
            {
        ...
        void EmpresaUm::Empresa::PromoverEmpregado(string id)//Faz Varias Checagens e move o Empregado, cujo ID foi passado de argumento, de Database mudando algumas de suas informacoes
        {
            int check = 0;
         //Procura todos os funcionarios do setor FINANCAS e se encontra o id deleta o funcionario da database
            for(unsigned int i = 0; i<financas.Auxiliares.size();i++)
            {
                if(financas.Auxiliares[i].Getid() == id)        //Se id do empregado eh igaul ao id passado
                {
                    ...
                    {                                           //ascender aos empregados de fun��o sempre que cumpram com ter a profiss�o adequada (por exemplo um auxiliar pode passar a sert�cnico, sempre que tenha estudado um t�cnico),
            ...
            for(unsigned int i = 0; i<financas.Tecnicos.size();i++)                      //Identico ao bloco de cima mas vai de Tecnico para Profissional e da uma profissao ao empregado que faca parte das profissoes do setor
            {
            ...
            for(unsigned int i = 0; i<financas.Profissionais.size();i++)                 //Promove Profissional para diretor de setor se nao existir um diretor de setor. Pois o diretor deve ser unico
            {
                if(financas.Profissionais[i].Getid() == id)
                {
                    check = 1;
           ...
           //Procura todos os funcionarios do setor MARKETING e se encontra o id deleta o funcionario da database
            ...
            for(unsigned int i = 0; i<marketing.Auxiliares.size();i++)                                               // SE REPETE PARA TODOS OS SETORES
            {
            ...
            for(unsigned int i = 0; i<DiretoresdeOperacao.size();i++)
            {
        ...
        void EmpresaUm::Empresa::CalcularPorcentagem()
        {
            ...
            //Conta todos os funcionarios do setor FINANCAS====================================================================================================
            for(unsigned int i = 0; i<financas.Auxiliares.size();i++)                                                    //Varre todas as databases
            {
        ...
        string EmpresaUm::Empresa::EncontrarEmpregado(string id)
        {
            int check =0;                                           //Serve para checar se o empregado foi encontrado. Se ele permanecer 0 significa que nao existe empregado com esse ID
          //Procura todos os funcionarios do setor FINANCAS e se encontra o id deleta o funcionario da database
            for(unsigned int i = 0; i<financas.Auxiliares.size();i++)        //Para cada setor e tipo de empregado os blocos funcionam da seguinte maneira
