...
EmpresaUm::Empregado::Empregado(string nome, string idade, string dataNasc, string CPF, string ID, string dataEmpresa, string titulo,string prof1,string prof2,string prof3, string cargo, float salario):pertence(new Setor(titulo,prof1,prof2,prof3))
{
   EmpresaUm::Empregado::Setnome(nome);
   EmpresaUm::Empregado::SetdataNasc(dataNasc);        //Data de nascimento antes da idade pois ela pode sofrer correcao
   EmpresaUm::Empregado::Setidade(idade);
   EmpresaUm::Empregado::Setcpf(CPF);
   EmpresaUm::Empregado::Setid(ID);
   EmpresaUm::Empregado::SetdataEntradaEmpresa(dataEmpresa);
   EmpresaUm::Empregado::Setcargo(cargo);
   EmpresaUm::Empregado::Setsalario(salario);
}
...
EmpresaUm::Empregado::~Empregado()
{
        ...
        void EmpresaUm::Empregado::Imprime()   //Imprime na tela as informacoes do objeto
        {
        cout <<"Nome: "<< nome << endl;
        ...
        void EmpresaUm::Empregado::Aumentarsalario(double val)       //Aumenta o salario do empregado pela quantia passada
        {
...
void EmpresaUm::Empregado::Setidade(string idadedigitada)          //Muda a idade do empregado e modifica o ano da data de nascimento se ela estiver incorreta
{
    int check = 0;
    ...
    if (check ==0)
    {
    ...
    else
     {
         cout << " A idade digitada nao possui caracteres validos" << endl;
            ...
            void EmpresaUm::Empregado::Setnome(string val)
            {
                int check = 0;
        ...
        void EmpresaUm::Empregado::Setcpf(string val)
        {
        int check=0;
        if (val.size() == 11)
        {
        ...
        void EmpresaUm::Empregado::SetdataNasc(string val)
        {
        int check=0;
        if (val.size() == 10)
        {
        ...
        void EmpresaUm::Empregado::SetdataEntradaEmpresa(string val)
        {
        int check=0;
        if (val.size() == 10)
        {
