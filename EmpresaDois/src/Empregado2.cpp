...
namespace EmpresaDois {
  EmpresaDois::Empregado::Empregado(string nome, string idade, string dataNasc, string CPF, string ID, string dataEmpresa, string titulo,string prof1,string prof2,string prof3, string cargo, float salario):pertence(new Setor(titulo,prof1,prof2,prof3))
{
   EmpresaDois::Empregado::Setnome(nome);
   EmpresaDois::Empregado::SetdataNasc(dataNasc);        //Data de nascimento antes da idade pois ela pode sofrer correcao
   EmpresaDois::Empregado::Setidade(idade);
   EmpresaDois::Empregado::Setcpf(CPF);
   EmpresaDois::Empregado::Setid(ID);
   EmpresaDois::Empregado::SetdataEntradaEmpresa(dataEmpresa);
   EmpresaDois::Empregado::Setcargo(cargo);
   EmpresaDois::Empregado::Setsalario(salario);
}

EmpresaDois::Empregado::~Empregado()
{

}

        void EmpresaDois::Empregado::Imprime()   //Imprime na tela as informacoes do objeto
        {
        cout <<"Nome: "<< nome << endl;
        ...
        void EmpresaDois::Empregado::Aumentarsalario(double val)       //Aumenta o salario do empregado pela quantia passada
        {
...
void EmpresaDois::Empregado::Setidade(string idadedigitada)          //Muda a idade do empregado e modifica o ano da data de nascimento se ela estiver incorreta
{
    int check = 0;
    for (unsigned int j = 0; j < idadedigitada.size(); j++){
    ...
    if (check ==0)
    {
    ...
    else
     {
         cout << " A idade digitada nao possui caracteres validos" << endl;
            ...
            void EmpresaDois::Empregado::Setnome(string val)
            {
                int check = 0;
            for (unsigned int j = (val.size()-1); j >= 0; j--){
        ...
        void EmpresaDois::Empregado::Setcpf(string val)
        {
        int check=0;
        if (val.size() == 11)
        {
        ...
        void EmpresaDois::Empregado::SetdataNasc(string val)
        {
        int check=0;
        if (val.size() == 10)
        {
        ...
        void EmpresaDois::Empregado::SetdataEntradaEmpresa(string val)
        {
        int check=0;
        if (val.size() == 10)
        {
